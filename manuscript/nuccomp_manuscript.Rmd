...
Bioinformatic projects frequently begin with a large FASTA or FASTQ format file, such as a genome.
...
We present nuccomp as a tool to query and present summaries of the contents of FASTA files containing nucleotides.
...
With nuccomp we can rapidly assess the contents of FASTA format files so we can move on to downstream procedures with confidence in knowing what the file contains.
...
The repository nuccomp contains a python script, an RMarkdown document, example data, and example output.
The python script takes a FASTA or FASTQ format file, consisting of nucleotides, as input.
The output from the python script is summarized graphically and with tables using the RMarkdown document.
...
This script may be run independently or may be called from the RMarkdown script.
...
The repository nuccomp is currently available on GitHub ([https://github.com/knausb/nuccomp](https://github.com/knausb/nuccomp)).
...
The RMarkdown script ```nuccomp.Rmd``` reads in this tabular data and demonstrates how to create individual plots for information from the output of the python script and how to make a publication quality composite plot summarizing multiple data features.
An example of this plot is presented in Figure 1.
Execution of this RMarkdown script, including checks for dependencies, the call to the python script, and the rendering of graphics, required less than 10 seconds.
