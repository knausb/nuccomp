...
Tools for summarizing bioinformatic files exist (Table 1), but they are currently distributed among many software packages designed to work on various file formats.
...
```{r, echo=FALSE}
my_dat <- data.frame(
  Name = c("FastQC", "trimmomatic", "BUSCO", "Blobtools"),
  Author = c("Andrews et al. 2010", "Bolger, Lohse and Usadel 2014", " Simão et al. 2015", "Laetsch and Blaxter 2017")
)

knitr::kable(my_dat, caption = "**Table 1.** Existing software that describe the contents of FASTA/Q files.")
```
...
The GitHub repository nuccomp (https://github.com/knausb/nuccomp) contains two python scripts, an RMarkdown document (Table 2), example data, and example output.
...
The study of genomics could currently be described as the study of biological sequence files in the FASTA and FASTQ formats.
These investigations begin with the receipt of these files from data centers or collaborators.
This presents a need to rapidly determine the contents of these files.
We present the repository nuccomp which includes software to rapidly summarized the content of FASTA and FASTQ files.
These summaries are further processed in RMarkdown as graphical presentations and with the hope that the use of RMarkdown will inspire users to customize these reports.
The use of nuccomp is expected to facilitate investigations into individual genomes, such as genome announcement style investigations, as well as scale to many genomes, such as pan-genomic projects.
