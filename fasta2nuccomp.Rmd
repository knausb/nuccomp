...
Note that the end user may have to adjust the manual placement of text in each plot.
...
puc <- sum(my_nucs[,c("A")])/sum(rowSums(my_nucs[,c("a","A")]))
...
pa <- pa + annotate("text", x = 0.95, y=1e1,
                    ...
                    adj = 1)
pa <- pa + annotate("text", x = 0.95, y=8.0,
                    label= paste("Percent uppercase:", format(puc, nsmall = 2)),
                    adj = 1)
pa
```
...
pc <- pc + annotate("text", x = 0.95, y=1e1,
                    label= paste("Median:", round(my_med, digits = 3)),
                    adj = 1)
pc <- pc + annotate("text", x = 0.95, y=8,
                    label= paste("Percent uppercase:", format(puc, nsmall = 2)),
                    adj = 1)
pc
```
...
puc <- sum(my_nucs[,c("G")])/sum(rowSums(my_nucs[,c("g","G")]))
...
pg <- pg + annotate("text", x = 0.95, y=1e1,
                    label= paste("Median:", round(my_med, digits = 3)),
                    adj = 1)
pg <- pg + annotate("text", x = 0.95, y=8,
                    label= paste("Percent uppercase:", format(puc, nsmall = 2)),
                    adj = 1)
pg
```
...
puc <- sum(my_nucs[,c("T")])/sum(rowSums(my_nucs[,c("t","T")]))
...
pt <- pt + annotate("text", x = 0.95, y=1e1,
                    label= paste("Median:", round(my_med, digits = 3)),
                    adj = 1)
pt <- pt + annotate("text", x = 0.95, y=8,
                    label= paste("Percent uppercase:", format(puc, nsmall = 2)),
                    adj = 1)
pt
```
