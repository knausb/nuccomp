...
ggarrange(
  ggarrange(pa, pc, pt, pg, ncol = 2, nrow = 2),
  pl, 
  ggarrange(pu, pn, pe, ncol = 3, nrow = 1),
  ncol = 1, nrow = 3,
  heights = c(2.5, 2, 1))
...
For the purpose of publication we may desire to save this "dashboard."
The ```ggsave()``` function can be uncommented, and the filename modified, in order to save this dashboard as a ```*.png``` or ```*.tiff``` graphic.
